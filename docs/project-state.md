...
**`Portable Clone Test` — resolved 2026-07-11:** этот job намеренно не делает `actions/checkout` (клонирует репо в случайный `/tmp/ccip-<uuid>`, чтобы проверить портативность `audit-suite`) — из-за этого `GITHUB_WORKSPACE` пуст, а `pnpm/action-setup`/`actions/setup-node` умеют резолвить `package_json_file`/`cache-dependency-path` только относительно него (задокументированный контракт, во всех примерах доки — только с предшествующим checkout). Прошлая попытка фикса (`b0ed553`, 2026-06-16) передала в `package_json_file` абсолютный путь клона — по контракту экшена это не работает (тихо резолвится в «ничего»), тот фикс так и не был верифицирован (CI вскоре заблокировал биллинг). Исправлено двумя коммитами: `5c30daa` (явный `version: 9` вместо file-detection) и `916dfcc` (убран `cache: pnpm`/`cache-dependency-path` — редкому nightly-смоук-тесту кэш не нужен). Подтверждено вручную через `gh workflow run` (run 29162495228): `audit-suite` реально прогнался из случайного пути, 22/22 passed.
