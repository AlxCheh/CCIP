...
| **W8** | Dev-runtime bring-up + schema/migration sync | вскрыто 2026-05-29 при M-05b Task 10 (первый E2E прогон стека) | Почти done: runtime bring-up (`40308e7` — @ccip/database CJS build, BullMQ REDIS_PASSWORD, @Public GP submit, seed bcrypt/import; pg_cron PR #9) + schema/code drift B-01/B-03/B-04/B-05/B-06 (`c8e8292`,`4686af2`). **Scenario A E2E green.** Остаётся только **B-02** migration-history baseline (не блокирует функциональность) | 0.5 d (B-02) | `prisma migrate deploy` на чистой БД |
