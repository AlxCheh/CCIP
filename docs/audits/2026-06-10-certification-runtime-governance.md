...
| **R-1** | ~~MINOR~~ ✅ **CLOSED** | flush-state + post-agent-hook | ~~Тихий откат к .bak без alert~~ → corrupt-main recovery пишет governance_alert (recovered/lost) + stderr; reactor surface'ит (R-1↔G-1) | исполнено: corrupt main → alert → reactor directive |
